...
> **注意，我成绩出完了，现在已经把workflow里面的时间给改动了抑制它别跑了，如果你要再fork一下的话，记得把 `.github/workflows/*.yml` 里面注释掉的第四个schedule给取消注释。**
...
- **标题（喜/没变/悲）**：基于这次抓取后的总 GPA 和上一次相比是升高、不变还是降低，给出一个直观的情绪化提示。首次运行会显示"首次获取成绩"。一般来讲“不变”应该就是出了一门PNP的课的成绩。
- **课程变动详情**：列出本次检测到的所有差异，分为三类。
  - `新课程`：上一次快照里不存在、这次新出现的课程记录
  - `成绩变动`：同一门课的绩点相比上次发生了变化（比如在相邻自动运行周期内，老师撤回了成绩，重新发布后同时把你的绩点给抬高了）
  - `课程被删除`：上一次存在但这次成绩单里找不到的课程（一般极少出现，我以前只遇见过一次老师撤回成绩的）
- **成绩单摘要**：按学期列出 GPA 和课程数，以及总 GPA、总课程数，方便快速了解整体情况，不需要逐条翻看课程列表。

注意当前针对P/NP课程。我统一写成了GPA为0（一般应该不会NP吧:(）.要是觉得不放心再登到教务网站上去看看。懒得专门写个判断了，后面闲得无聊可以改改。
...
- 请遵守学校教务系统的使用条款，自动化抓取行为请控制合理的访问频率，避免对教务服务器造成不必要的负担。
